...
@section('title', 'Laporan Barang Masuk Vendor | Ethica')
@section('content')
<div class="card">
    @include('flash-message')
    <div class="card-header">
        <h1 class="card-title" style="float: left; font-size: 20pt">Laporan Barang Masuk Vendor</h3>
    ...
    <form id="penjualan" class="form-horizontal" method="get" action="{!! route('cari_barang_masuk_vendor') !!}">
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                <div class="col-md-2">
            ...
            <div class="card-footer">
                <a href="{!! route('lap_barang_masuk_vendor') !!}" class="btn btn-danger btn-sm"><span class="fa fa-times"></span> Reset</a>
        ...
        <div class="card card-primary card-tabs">
            <div class="card-header p-0 pt-1">
                <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" id="custom-tabs-one-home-tab" data-toggle="pill" href="#custom-tabs-one-home" role="tab" aria-controls="custom-tabs-one-home" aria-selected="true">Laporan Barang Masuk Vendor</a>
                    </li>
                    <!--<li class="nav-item">
                                        ...
                                        <div class="card-body table-responsive p-0">
                                            <table id="exam" class="table table-hover text-nowrap" style="font-size: x-small">
                                                ...
                                                <tbody>
                                                    ...
                                                    $qty = 0;
                                                    ?>
                                                    @if(!empty($barangmasukvendor))
                                                    @foreach($barangmasukvendor as $barangmasukvendors)
                                                    <?php $no++; ?>
                                                    <tr>
                                                        <td class="bg-gradient-gray">{!! $no !!}</td>
                                                        <td class="bg-gradient-gray">{!! $barangmasukvendors->tanggal !!}</td>
                                                        <td>{!! $barangmasukvendors->nomor !!}</td>
                                                        <td>{!! $barangmasukvendors->nmcustomer !!}</td>
                                                        <td>{!! $barangmasukvendors->barcode !!}</td>
                                                        <td>{!! $barangmasukvendors->nmproduk !!}</td>
                                                        <td>{!! $barangmasukvendors->nmbrand !!}</td>
                                                        <td>{!! $barangmasukvendors->nmsubbrand !!}</td>
                                                        <td>{!! $barangmasukvendors->nmwarna !!}</td>
                                                        <td>{!! $barangmasukvendors->nmukuran !!}</td>
                                                        <td style="text-align: right">{!! number_format($barangmasukvendors->qty,0) !!}</td>
                                                        <td>{!! $barangmasukvendors->status !!}</td>
                                                        <td>{!! $barangmasukvendors->nmadmin !!}</td>
                                                    </tr>
                                                    <?php
                                                    $qty += ($barangmasukvendors->qty);
                                                    ?>
                                                    @endforeach
                                                    @endif
                                                </tbody>
