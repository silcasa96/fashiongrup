...
@section('title', 'Laporan histori Login | Ethica')
@section('content')
<div class="card">
    @include('flash-message')
    <div class="card-header">
        <h1 class="card-title" style="float: left; font-size: 20pt">Laporan Histori Login</h3>
    ...
    <form id="penjualan" class="form-horizontal" method="get" action="{!! route('cari_histori_login') !!}">
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                <div class="col-md-2">
    ...
    <div class="col-sm-12">
        <div class="card card-primary card-tabs">
            <div class="card-header p-0 pt-1">
                                <h3 class="card-title">Laporan Histori Login</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body table-responsive p-0">
                                <table id="exam" class="table table-hover text-nowrap" style="font-size: x-small">
                                    ...
                                    <tbody>
                                        ...
                                        $total = 0;
                                        ?>
                                        @if(!empty($histori_login))
                                        @foreach($histori_login as $histori_logins)
                                        <?php $no++; ?>
                                        <tr>
                                            <td>{!! $no !!}</td>
                                            <td>{!! date('d/m/Y',strtotime($histori_logins->tanggal)) !!}</td>
                                            <td>{!! $histori_logins->user_id !!}</td>
                                            <td style="text-align: right">{!! $histori_logins->jml_login !!}</td>
                                        </tr>
                                        <?php
                                        $total += ($histori_logins->jml_login);
                                        ?>
                                        @endforeach
                                        @endif
                                    </tbody>
